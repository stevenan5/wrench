...
from wrench.labelmodel import AMCL_CC
...
def run_amcl_cc(
    ...
    for run_no in range(n_runs):
        ...
        label_model = AMCL_CC(logger)
        for _ in range(n_fit_tries):
            start_time = perf_counter()
            cnd = label_model.fit(train_data, valid_data, n_samples=num_samples)
        ...
        # only run calibration code if num_class is 2
        if num_class == 2:
        ...
        mdic["best_theta"].append(label_model.model_theta)
    ...
    if n_runs > 1:
        ...
        logger.info('Average time to fit %.1f (std: %.1f)',
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        # make folder for AMCL_CC specifically
        method_result_path = os.path.join(dataset_result_path, 'AMCL_CC')
